...
def test_version_flag_gives_correct_result():
    result = runner.invoke(cli.app, ["--version"])
    assert result.exit_code == 0
    assert f"{__app_name__} v{__version__}\n" in result.stdout


def test_v_flag_gives_same_result_as_version():
    version_result = runner.invoke(cli.app, ["--version"])
    v_result = runner.invoke(cli.app, ["-v"])
    assert v_result.exit_code == 0
    assert v_result.stdout in version_result.stdout
